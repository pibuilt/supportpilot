...
export function AdminPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <h3 className="text-lg font-semibold text-slate-950">Tenant API keys</h3>
        <p className="mt-1 text-sm text-slate-500">
          Revoke or regenerate tenant-scoped keys directly from the admin console.
        </p>
        <div className="mt-5 space-y-3">
          {apiKeysQuery.data?.length ? (
            apiKeysQuery.data.map((entry) => (
              <div key={entry.api_key_id} className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      {entry.owner} • {entry.role} • {entry.is_active ? "active" : "revoked"}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="secondary" onClick={() => actionMutation.mutate({ action: "revoke-key", targetId: entry.api_key_id })}>
                      Revoke
                    </Button>
                    <Button type="button" onClick={() => actionMutation.mutate({ action: "regenerate-key", targetId: entry.api_key_id })}>
                      Regenerate
                    </Button>
                  </div>
                </div>
            ...
            <EmptyState title="No API keys returned" description="Tenant API keys will appear here once they have been issued." />
          )}
        </div>
        {regeneratedKey ? (
