  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Product routes require `x-api-key`. The backend only returns the full key on initial issuance.
            </p>
          </div>
      ...
      <Card>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Fresh logins may not receive the original full API key because the backend returns a placeholder when an active key already exists.
        </p>
        ...
        <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          The backend currently exposes validation but not user-facing API key creation or listing endpoints. This page reflects that limitation instead of inventing unsupported flows.
        </div>
      </Card>
    </div>
  );
